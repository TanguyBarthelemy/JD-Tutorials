...
There are two more versions of JDemetra+ which are only at a **test** stage:
...
JDemetra+ v2.2.3 and v2.2.4 require Java version $\geq 8$ while v3.0.0 requires Java version $\geq 17$:
...
-   In the unzipped folder, **open** (for example with Notepad++) the file `jwsacruncher.bat` present in the subfolder `r colFmt("\\bin\\", windows_path_color, url = FALSE)` (that is under `r colFmt("jdemetra-cli-2.2.3\\bin\\", windows_path_color, url = FALSE)` in the version 2.2.3 of the cruncher)
...
To install a package on AUS, you can't use the function `install_github()`{.r}. Therefore, if the package is not on CRAN, it must be downloaded at the binary format (`.zip`)
