...
Aussi on peut préciser les variables d'environnement `http_proxy` et `https_proxy` pour Windows.
...
Si une nouvelle version de Java a été installé (Java 17 par exemple), il faut préciser à Rstudio la localisation de l'installation. Pour cela, comme pour paramétrer le proxy :
...
Aussi on peut préciser la variable d'environnement `JAVA_HOME` pour Windows.
