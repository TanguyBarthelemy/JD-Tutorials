...
-   [v3.2.1](https://github.com/jdemetra/jdplus-main/releases/tag/v3.2.1) : la nouvelle version de JDemetra+ avec de nouvelles fonctionnalités et une nouvelle interface

La v2.2.4 de JDemetra+ nécessite une version de Java $\geq 8$ alors que la v3.2.1 nécessite une version de Java $\geq 17$ :
...
ℹ️ Conseil : si vous comptez utiliser plusieurs version de JDemetra+ (v2.2.4, v3.2.1, ...), vous pouvez renommer les dossiers dézippés en `r color_format("\\nbdemetra-2.2.4\\", color = windows_path_color, url = FALSE)` et `r color_format("\\nbdemetra-3.2.1\\", color = windows_path_color, url = FALSE)`.
...
La version 3.2.1 de JDemetra+ contient une jdk 17 (version de java 17) packetée dans le [.zip](https://github.com/jdemetra/jdplus-main/releases/tag/v3.2.1). Ainsi pour utiliser l'interface, il n'est pas nécessaire d'installer java 17.

En revanche, pour utiliser les packages R en version 3 sans télécharger la GUI (en version 3.2.1), il est obligatoire d'avoir java >= 17 et donc de l'installer soit même.
...
Exemple pour le package [**{rjd3toolkit}**]{.RPackages}, le `Source code (tar.gz)` se trouve sous `r color_format("https://github.com/rjdemetra/rjd3toolkit/releases/tag/v3.1.0", color = html_color, url = TRUE)` (*release* Section). Après l'avoir récupéré, il faut lancer la commande d'installation :
