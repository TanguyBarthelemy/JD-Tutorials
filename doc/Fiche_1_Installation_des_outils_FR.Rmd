...
colFmt <- function(x, color, url = TRUE) {
    ...
    if (outputFormat == 'latex') {
        ...
        ret <- x |> 
            gsub(pattern = "\\\\", replacement = " \\backslash ", fixed = TRUE)
        ret <- paste(ifelse(url, "[", ""), 
                     "$\\textsf{\\color{", color, "}", ret, "}$", 
                     ifelse(url, paste0("](", x, ")"), ""), 
                     sep = "")
    } else {
        ret <- x
...
Exemple pour le package **rjd3toolkit**, le `Source code (tar.gz)` se trouve sous `r colFmt("https://github.com/rjdemetra/rjd3toolkit/releases/tag/v3.0.0", color = html_color, url = TRUE)` (*release* Section). Après l'avoir récupéré, il faut lancer la commande d'installation :
