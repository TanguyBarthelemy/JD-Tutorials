...
On computers without administrator rights (*professional computer, for
example*), it is recommended to create a folder **Software** under
...
- **Copy paste** this value after `PATH =` and add the paths towards the
  ...
  For the Rtools installation, the path is
...
Finally, you can display the Java version installed with which Windows
...
To check that the path is valid, you can use the function
`getOption(...)` :
